...
from .serializers import patientRegistrationSerializer, patientProfileSerializer, patientHistorySerializer, appointmentSerializerPatient
...
class registrationView(APIView):
    ...
    def post(self, request, format=None):
        registrationSerializer = patientRegistrationSerializer(data=request.data.get('user_data'))
        profileSerializer = patientProfileSerializer(data=request.data.get('profile_data'))
...
class appointmentViewPatient(APIView):
    ...
    def post(self, request, format=None):
        ...
        user_patient = patient.objects.filter(user=user).get()
        history = patient_history.objects.filter(patient=user_patient).latest('admit_date')
        serializer = appointmentSerializerPatient(data=request.data)
